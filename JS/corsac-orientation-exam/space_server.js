...
app.post('/movehere', function(req, res) {
  let select1 = `SELECT name FROM planet WHERE id = ${req.query.planet_id}`;
  conn.query(select1, function(err, rows) {
    if (err) {
      console.log(err.toString());
      res.status(500).send('Database error');
      return;
    }
    let select2 = `UPDATE spaceship SET planet = "${rows[0].name}" WHERE id = "1"`;
